...
parser = argparse.ArgumentParser(
    description="FLUIDS First Order Lightweight Urban Intersection Driving Simulator",
    formatter_class=argparse.RawDescriptionHelpFormatter,
    epilog=key_help,
)
parser.add_argument(
    "-b", metavar="N", type=int, default=10, help="Number of background cars"
)
parser.add_argument(
    "-c", metavar="N", type=int, default=1, help="Number of controlled cars"
)
parser.add_argument(
    "-p", metavar="N", type=int, default=5, help="Number of background pedestrians"
)
parser.add_argument("-v", metavar="N", type=int, default=1, help="Visualization level")
parser.add_argument(
    "-o",
    metavar="str",
    type=str,
...
parser.add_argument(
    "--time", metavar="N", type=int, default=0, help="Max time to run simulation"
)
parser.add_argument(
    "--state",
    metavar="file",
    type=str,
    default=fluids.STATE_CITY,
...
fluids_print(
    "            Datasaver           : {}".format(
        "disabled" if not args.datasaver else "Storing file at " + args.datasaver
    )
)
fluids_print(
...
fluids_print(
    "            Pedestrian lights   : {}".format(
        "enabled" if args.pedlights else "disabled"
    )
)
fluids_print(
...
if args.datasaver != "":
    data_saver = fluids.DataSaver(
        fluid_sim=simulator, file_path=args.datasaver, batch_size=4
    )
