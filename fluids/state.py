...
class State(object):
    ...
    def __init__(
        ...
        cfilename = "{}{}.json".format(
            hashlib.md5(str(layout).encode()).hexdigest()[:10], __version__
        )
        ...
        for i in range(controlled_cars + background_cars):
            while True:
                ...
                min_d = min(
                    [car.dist_to(other) for k, other in iteritems(self.type_map[Car])]
                    + [np.inf]
                )
        ...
        for i in range(background_peds):
            while True:
                ...
                ped = Pedestrian(
                    state=self, x=wp.x, y=wp.y, angle=wp.angle, vis_level=vis_level
                )
        ...
        if vis_level:
            ...
            try:
                self.static_debug_surface = pygame.Surface(
                    self.dimensions, pygame.SRCALPHA
                )
            except ValueError:
                fluids_print(
                    "WARNING: Alpha channel not available. Visualization may be slow"
                )
    ...
    def generate_waypoints_init(self):
        self.waypoints = [
            lane.start_waypoint for k, lane in iteritems(self.type_map[Lane])
        ]
        self.waypoints.extend(
            [lane.end_waypoint for k, lane in iteritems(self.type_map[Lane])]
        )
        ...
        for k, street in iteritems(self.type_map[Street]):
            for waypoint in self.waypoints:
                if street.intersects(waypoint):
                    test_point = (
                        waypoint.x + np.cos(waypoint.angle),
            ...
            for kd, obj in iteritems(self.dynamic_objects):
                for ko, sobj in iteritems(self.objects):
                    if obj.collides(sobj):
                        pygame.draw.circle(
                            dynamic_surface, (255, 0, 255), (int(obj.x), int(obj.y)), 10
                        )
